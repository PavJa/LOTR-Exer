...
df_BooksUsersRate['Bracket'] = df_BooksUsersRate['Book-Title'].str.extract(r'\((.*)\)').astype('str')
df_BooksUsersRate['Book-Title'] = df_BooksUsersRate['Book-Title'].str.replace(r'\(.*\)', '',regex=True)
df_BooksUsersRate['Book-Title'] = df_BooksUsersRate['Book-Title'].str.strip()
df_BooksUsersRate['Bracket'] = df_BooksUsersRate['Bracket'].str.strip()
df_BooksUsersRate['Bracket'] = df_BooksUsersRate['Bracket'].str.replace(r'\(.*\)', '',regex=True)

#print(df_BooksUsersRate['Book-Title'].to_string())
#print(df_BooksUsersRate.loc[df_BooksUsersRate['Book-Title'] == 'lord of the rings'].to_string())
#print(df_BooksUsersRate.loc[df_BooksUsersRate['Bracket'] == 'lord of the rings'].to_string())
...
Tolkien_list_rating = [x for x, y, z, b in zip(df_BooksUsersRate['User-ID'],df_BooksUsersRate['Book-Title'],
                                          df_BooksUsersRate['Book-Rating'], df_BooksUsersRate['Bracket']) if
                       (('lord of the rings' in b or 'the lord of the rings' == y) and z >= 7)]
...
for x in Tolkien_list_rating:
    k = df_BooksUsersRate.loc[(df_BooksUsersRate['User-ID'] == x) & (df_BooksUsersRate['Book-Rating'] >= 7) &
                              (10 <= df_BooksUsersRate['Age']) & (df_BooksUsersRate['Age'] <= 80)]
...
df_union_grouped_count = df_union_grouped_count[df_union_grouped_count['Count'] > 40]
